        ...
        for image in image_ids_copy:
            ...
            for k in range(K):
                ...
                if (centroids_attach > 0):
                    ...
                    if(len(centroids_ifnotFound)>0):
                        centroids_dict[0][list(image_ids).index(centroids_ifnotFound[randint(0, len(centroids_ifnotFound)-1)])].append(adj_mat[list(image_ids).index(centroids_ifnotFound[randint(0, len(centroids_ifnotFound)-1)])][list(image_ids).index(image)])
                        centroids_dict[1][centroids_ifnotFound[randint(0, len(centroids_ifnotFound)-1)]].append(image)
                        image_ids_copy.remove(image)
                        flag = 1
        i=i+1
